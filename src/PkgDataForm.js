...
const PkgDataForm = () => {
    ...
    const isAnswerMatch = (fieldDependency, parentAnswer) => {
        ...
        const conditions = fieldDependency.split(/OR/i).map((dep) => normalizeText(dep.trim()));
    ...
    const renderQuestions = (questions) => {
        return questions.map((question) => {
            ...
            const isDependentVisible = isAnswerMatch(question.field_dependency, parentAnswer);
    ...
    const renderSections = () => {
        return Object.entries(sections).map(([section, questions]) => {
            ...
            return (
                <div className="form-section mt-5" key={section} ref={sectionRefs.current[section]}>
            ...
            <div className="container-fluid px-5 py-58">
                <div className="row">
                    <div className="col-12 col-md-3">
                        <div className="p-4 bg-color-light-gray">
                            <span className="fs-12 fw-600 text-color-typo-secondary">
                                Complete all sections for this component
                            </span>
                            {Object.keys(sections).map((section) => (
                                <p
                                    className="fs-14 fw-600 text-color-typo-secondary cursor-pointer"
                                    key={section}
