...
const AdminComponentPage = () => {
    const [skuComponentDataIncoming, setSkuComponentDataIncoming] = React.useState({});
    ...
    const getSKUComponentDataDetails = async () => {
      ...
      try {
        ...
        const selectComponentData = skuComponentData?.filter((item)=> item?.id == Number(componentId))[0];
    ...
    <div>
      ...
      <div className="d-flex">
        ...
        <div className="mainContent-holder w-100 h-100 bg-color-light-gray-shade">
          <ComponentSubHeaderStrip skuComponentDataIncoming={skuComponentDataIncoming} componentsAllId={componentsId} setLoading={setLoading} setApiCallAgain={setApiCallAgain}/>
          {/* Main Section */}
          <div className="container-fluid px-20 px-md-4 pt-30 container-height d-flex flex-column">
            <div className="row">
              <ComponentSidePanel groupingSectionName={groupingSectionData && Object.keys(groupingSectionData?.sections || {})} activeSection={activeSection} setActiveSection={setActiveSection}/>
              <ComponentForm questionAvailable={questionAvailable} skuComponentDataIncoming={skuComponentDataIncoming} groupingSectionData={groupingSectionData?.sections} activeSection={activeSection}/>
            </div>
          </div>
        </div>
      </div>
