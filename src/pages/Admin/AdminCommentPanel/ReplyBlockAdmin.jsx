...
import { formatDate, nameFormation } from "./CommentRowParentMessage";
...
const ReplyBlockAdmin = (props) => {
  ...
  const handleDeleteCommentAPI = async (commentId) => {
    try {
      ...
      if (
        repliesMessages?.filter((item) => item?.is_deleted === false)
          ?.length === 0
      ) {
        setReplyBoxShow(false);
        setOpenReplies(false);
  ...
  return (
    <>
      ...
      {repliesMessages?.filter((item) => item?.is_deleted === false)?.length >
        0 &&
        repliesMessages
