...
const AdminSKUDetails = () => {
  ...
  const getSKUDetails = async () => {
    try {
      const response = await axiosInstance.get(`skus/${skuId}/?pko_id=${pkoId}`);
    ...
    <div>
      ...
      <div className="d-flex">
        ...
        <div className="mainContent-holder w-100 h-100 bg-color-light-gray-shade">
          <SKUDetailsSubHeaderStrip descriptionIncoming={skuDataIncoming?.description} statusIncoming={skuDataIncoming?.status} submitted_date={skuDataIncoming?.updated_date} setApiCallAgain={setApiCallAgain}/>
          {/* Main Section */}
          <div className="container-fluid px-20 px-md-4 pt-30 admin-container-height d-flex flex-column">
            ...
            <div className="row h-100">
              ...
              <SKUDetailsPrimaryPackagingDetails skuDetailsPrimaryPackageDetailData={skuDataIncoming?.primary_packaging_details}/>
              {/* Sku Components Section */}
              <SKUDetailsSKUComponents skuComponentDataIncoming={skuDataIncoming?.components}/>
            </div>
          </div>
        </div>
      </div>
